...
| `location_mismatch` | Whether the location doesn't match the cardholder's usual area |
...
**Key insight:** Only 1.5% of transactions are fraud (151 out of 10,000). This is called a **class imbalance** and requires careful handling.
...
```
**What to look for:**
- **Non-Null Count** — checks for missing values. All 10,000 rows are complete — no missing data.
...
**Why we do this:**
ML models are mathematical equations — they can't process text like "Food" or "Travel." Label Encoding converts text categories into numbers:
...
**Rule of thumb:** Computers process numbers, not language. Any text column must be converted before feeding it to a model.
...
**Why explicitly list features:** Safer than `df.drop('is_fraud')` — if new columns are added to the dataset later, your X won't accidentally include something unintended.
...
**What this does:**
- **80% (8,000 rows)** → training data — the model learns from this
- **20% (2,000 rows)** → test data — the model is evaluated on this data it has never seen

**Analogy:** Like studying for an exam — the 8,000 rows are your study material, the 2,000 are the actual test.

**`random_state=42`** — ensures the split is reproducible. Anyone running the same code gets the same split.
...
**`model.fit(X_train, y_train)`** — this is where the actual learning happens. The model studies the training data and finds patterns.
...
- Fraud precision = 1.00 → Every time it said fraud, it was right. Zero false alarms.

**Recall** — Of all the actual fraud cases, how many did the model catch?
- Fraud recall = 0.61 → It caught 61% of real fraud. Missed 39%.

**F1 Score** — The balance between precision and recall. Useful for imbalanced datasets.

**Accuracy** — Overall correct predictions. 99% sounds great but is misleading here because 98% of data is already legitimate.

> **Key insight:** In fraud detection, **recall matters most.** Missing actual fraud costs the bank real money. A model that catches more fraud even with a few false alarms is more valuable than a precise but blind model.
...
- **12** — Fraud cases the model MISSED (dangerous)
...
**What it does:** Creates synthetic fraud examples to balance the dataset — 7,880 legitimate vs 7,880 fraud.
...
**Result:** Precision stayed at 100% but recall dropped to 55%. Slightly worse than original.
...
**Winner: Original model** — highest recall, perfect precision, best F1 score.

> **Lesson:** More complexity doesn't always mean better results. Sometimes the simplest model wins.
...
**On imbalanced data:**
A dataset where only 1.5% of cases are positive (fraud) will produce misleading accuracy scores. A model predicting everything as negative achieves 98.5% accuracy while being completely useless. Always evaluate with recall and F1 score in these cases.

**On choosing the right metric:**
The right metric depends on the business problem. In fraud detection, missing actual fraud is more costly than a false alarm; so recall is prioritized. In other problems the trade-off may be different. Always ask: *what does a wrong prediction actually cost?*
